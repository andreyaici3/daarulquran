...
{
	$ci = get_instance();
	$ci->form_validation->set_rules('kelas','Kelas','required|is_unique[tbl_kelas.kelas]',['is_unique' => 'Kelas Sudah Ada Dalam Database, Tidak Bisa Ditambahkan']);
}
...
{
	$ci = get_instance();
	$ci->form_validation->set_rules('nis','NIS','required|trim|max_length[12]');
	$ci->form_validation->set_rules('nama_siswa','Nama Siswa','required|trim');
	$ci->form_validation->set_rules('tempat_lahir','Tempat Lahir','required|trim');
	$ci->form_validation->set_rules('tanggal_lahir','Tanggal Lahir','required|trim');
	$ci->form_validation->set_rules('id_kelas','Kelas','required|trim');
	// $ci->form_validation->set_rules('foto_guru','Foto Guru','required|trim');
}
...
function fSukses($pesan,$href)
{
	$ci = get_instance();
	$ci->session->set_flashdata('msgi', '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>' . 
            $pesan
        . '</div>');
	redirect($href);
}
